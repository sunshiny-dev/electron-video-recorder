...
function bindEvents(){
    ...
    document.querySelector('#recControlContainer').addEventListener(
        'click',
        recording
    );
...
function startRecording(e){
    var record = document.querySelector('#recControlContainer');
    
    document.querySelector('#start').classList.add('hide');
    document.querySelector('#stop').classList.remove('hide');
    record.classList.remove('start');
    
    var el = document.getElementById('videoContainer');
    controls = new Recorder(el);
    controls.start();
}
...
function stopRecording(e){
    ...
    var store = __dirname + '/test/' + getDateTime();
    console.log(store);
    controls.stop();
    controls.save(store, 'mp4');
    controls.clear();
}
